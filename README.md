...
一款40%由 **ai** 制作的实时多人在线战术数字对决游戏。玩家通过**模 10 加法运算**操控手牌数值（0–9），触发各类武器、组合技和圣遗物能力，以智取胜。 
...
| 数据库 | JSON 平面文件 (`database.json`) | 用户档案、统计、库存持久化 | 
...
加法结果会改变你选中的那张手牌的数值，不同数值对应不同的武器/效果。 
...
在 2v2 团队模式下，组合技可以施加给队友。 

### 游戏模式

- **经典模式 (Classic)**：2-4 人自由对战，最后存活者获胜
- **团队模式 (Team)**：2v2 对战，按加入顺序分队（1-2 号 vs 3-4 号），一方全灭则另一方获胜 
...
玩家可装备一个圣遗物芯片，改变局内战术逻辑。所有圣遗物继承自 `RelicBase` 基类，通过钩子函数介入游戏各阶段。
...
| LEGENDARY | 💻 黑客入侵 | 空手(2)变渗透门户，积攒渗透值可抵消伤害或释放终极技能 |
...
- 新注册用户获赠 200 CR + 欢迎邮件（含 32,000 CR 及物资）
...
可获取物品包括：战术称号、特工边框、全息卡槽、倒计时法阵、战术模块等。
...
| 火力倾泻 | 累计造成 20 点伤害 | 80 CR | 

## 社交系统

- **好友系统**：对局内右键/长按对方头像发送/接受好友请求，查看好友在线状态
- **私信 (PM)**：与好友实时聊天
- **CR 转账**：向好友转移信用点（10% 手续费）
- **特工档案**：查看任意玩家的胜率、对局数、段位、签名
- **邮件系统**：收发邮件，支持附件（物品/CR），一键领取
- **房间内聊天**：对局中实时文字聊天
- **表情系统**：对局中点击己方头像发送快捷表情（❓💀🥶🔥）
